...
import AdminCardSearch from "./AdminCardSearch";
import { Card } from "@/types/card";
import CardsTable from "./CardTable";

export default function AdminCardsPageComponent() {
  ...
  const fetchCards = async () => {
    try {
      ...
      const res = await fetch(`/api/admin/cards?${params.toString()}&page=${page}`);
  ...
  if (loading === "loading" && cards.length === 0) {
    return (
      <div className="flex justify-center items-center h-full">
        <p className="ml-4 text-white">
          {t.admin.cardsPage.loading} <Spinner className="inline mx-0.5" />
        </p>
      </div>
    );
  }
    ...
    <div className="flex flex-col gap-6 mx-2">
      ...
      <div className="flex items-center justify-between gap-3">
        ...
        <div className="flex items-center gap-2.5">
          ...
          {/* Premium Add Card button */}
          <a
            href={`/${locale}/admin/cards/create`}
            className="
      ...
      {/* Content area */}
      {loading === "loading" ? (
        // Inline spinner while re-fetching (search change / page change)
        ...
        <div className="flex flex-col items-center gap-4 min-h-64 justify-center">
          <p className="text-red-500">{t.admin.cardsPage.error}</p>
          <button
            onClick={fetchCards}
            className="bg-blue-600 text-white px-4 py-2 rounded"
          >
            {t.admin.cardsPage.retry}
          </button>
        </div>
      ) : loading === "empty" ? (
        // Empty state — no data for the current search
        <div className="flex flex-col items-center gap-3 min-h-64 justify-center text-center">
          <p className="text-white/50 text-sm">{t.admin.cardsPage.empty}</p>
          {search !== "" && (
            <button
        ...
        <>
          <CardsTable cards={cards} refresh={fetchCards} />
