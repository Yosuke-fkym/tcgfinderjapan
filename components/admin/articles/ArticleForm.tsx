...
export default function ArticleForm({
  ...
  const [isProtected, setIsProtected] = useState(
    initialData?.is_protected ?? false,
  );
  ...
  const uploadThumbnail = async (articleId: string): Promise<string | null> => {
    ...
    if (!res.ok || result.error)
      throw new Error(result.error || t.articleForm.toast.thumbnailUploadError);
      ...
      if (mode === "create") {
        const res = await fetch("/api/admin/articles", {
          ...
          body: JSON.stringify({
            ...
            ...(isProtected &&
              password.trim() && { password_hash: password.trim() }),
          }),
        });
        ...
        toast.success(
          submitStatus === "published"
            ? t.articleForm.toast.publishSuccess
            : t.articleForm.toast.draftSuccess,
        );
      ...
      if (mode === "edit" && initialData?.id) {
        ...
        const res = await fetch(`/api/admin/articles/edit/${initialData.id}`, {
          ...
          body: JSON.stringify({
            ...
            ...(isProtected &&
              password.trim().length > 1 && { password_hash: password.trim() }),
        ...
        const result = await res.json();
        if (!res.ok || result.error)
          throw new Error(result.error || t.articleForm.toast.genericError);
  ...
  const showPasswordInput =
    isProtected &&
    (mode === "create" || changePassword || !initialData?.is_protected);
      ...
      <Card className="border shadow-sm">
        <CardHeader>
          <CardTitle className="flex items-center gap-2 text-xl">
            <FileText size={20} />
            {mode === "edit"
              ? t.articleForm.title.edit
              : t.articleForm.title.create}
          </CardTitle>
          <CardDescription>
            {mode === "edit"
          ...
          <div className="flex flex-col gap-8">
            ...
            <div className="space-y-4">
              <h3 className="font-semibold text-gray-800">
                {t.articleForm.sections.basicInformation}
              </h3>
              <div className="grid gap-4">
                <div className="grid gap-2">
                  <Label htmlFor="title">
                    {t.articleForm.labels.title}{" "}
            ...
            <div className="space-y-4">
              <h3 className="font-semibold text-gray-800">
                {t.articleForm.sections.thumbnail}
              </h3>
              ...
              {!thumbnailToShow && (
                <label className="border-2 border-dashed rounded-xl p-6 flex flex-col items-center justify-center text-center cursor-pointer hover:bg-muted/50 transition">
                  ...
                  <p className="font-medium">
                    {t.articleForm.messages.thumbnailUpload}
                  </p>
                  <p className="text-sm text-muted-foreground mt-1">
                    {t.articleForm.messages.thumbnailFormats}
                  </p>
                  <input
                    type="file"
                    accept="image/*"
              ...
              {thumbnailToShow && (
                <div className="relative w-full max-w-sm aspect-video rounded-xl border overflow-hidden bg-muted group">
                  ...
                  <img
                    src={thumbnailToShow}
                    alt={t.articleForm.thumbnail.previewAlt}
            ...
            <div className="space-y-4">
              ...
              <div className="grid gap-2">
                <Label htmlFor="content">
                  {t.articleForm.labels.articleBody}{" "}
                  <span className="text-red-500">*</span>
                </Label>
                <TiptapEditor
                  value={content}
            ...
            <div className="space-y-4">
              <h3 className="font-semibold text-gray-800">
                {t.articleForm.sections.category}
              </h3>
              <div className="grid gap-2">
                <Label>
                  {t.articleForm.labels.category}{" "}
                  <span className="text-red-500">*</span>
                </Label>
                <Popover open={categoryOpen} onOpenChange={setCategoryOpen}>
                  <PopoverTrigger asChild>
                    ...
                    >
                      {selectedCategory
                        ...
                        : t.articleForm.categorySelector.select}
                  ...
                  <PopoverContent className="p-0 w-64">
                    <Command>
                      <CommandInput
                        placeholder={
                          t.articleForm.placeholders.searchCategories
                        }
                      />
                      <CommandEmpty>
                        {t.articleForm.messages.noCategories}
                      </CommandEmpty>
                      <CommandList>
                        <CommandGroup>
                          {categories.map((cat) => (
              ...
              <div className="grid gap-2">
                ...
                <Popover open={tagOpen} onOpenChange={setTagOpen}>
                  <PopoverTrigger asChild>
                    ...
                    >
                      {selectedTagIds.length > 0
                        ? t.articleForm.dynamic.tagsSelected(
                  ...
                  <PopoverContent className="p-0 w-64">
                    <Command>
                      <CommandInput
                        placeholder={t.articleForm.placeholders.searchTags}
                      />
                      <CommandEmpty>
                        {t.articleForm.messages.noTags}
                      </CommandEmpty>
                      <CommandList>
                        <CommandGroup>
                          {tags.map((tag) => (
                      ...
                      <>
                        <div className="grid gap-2">
                          <Label htmlFor="article-password">
                            {mode === "edit"
                              ? t.articleForm.labels.newPassword
                              : t.articleForm.labels.password}
                            <span className="text-red-500"> *</span>
                          </Label>
                          <div className="relative">
                            ...
                            <button
                              ...
                              aria-label={
                                showPassword
                                  ? t.articleForm.protection.hidePassword
                                  : t.articleForm.protection.showPassword
                              }
                            >
                              {showPassword ? (
                                <EyeOff size={15} />
            ...
            <div className="flex justify-end gap-4 pt-4 border-t">
              ...
              >
                {submitting === "draft"
                  ? t.articleForm.actions.savingDraft
                  : t.articleForm.actions.saveDraft}
              </Button>
              ...
              >
                {submitting === "published"
                  ? mode === "edit"
                    ? t.articleForm.actions.updating
                    : t.articleForm.actions.publishing
                  : mode === "edit"
                    ? t.articleForm.actions.update
                    : t.articleForm.actions.publish}
              </Button>
            </div>
          </div>
        </CardContent>
