...
export default function MapSearchBar({
  ...
  const filteredAreas = AREA_OPTIONS.filter((a) => availableAreas.has(a.value));
  useEffect(() => {
  ...
  return (
    <div className="w-full mx-auto border-b-[0.5px] border-b-[#ffffff42] bg-[#08080b]/85 backdrop-blur-xl px-4 py-3 sticky top-16 z-40 shadow-sm">
      <div className="max-w-5xl lg:w-full w-full sm:w-[80%] mx-auto flex gap-2">
        ...
        <Popover open={open} onOpenChange={setOpen}>
          <PopoverTrigger asChild>
            <Button
              variant="outline"
              className="gap-2 flex cursor-pointer border-none aria-expanded:bg-indigo-600 aria-expanded:text-white bg-indigo-600 text-white hover:bg-indigo-700 shadow-md hover:text-white"
            >
              <SlidersHorizontal size={16} />
              <span className="sm:flex hidden">{t.map.filters.title}</span>
              {activeCount > 0 && (
              ...
              {filteredAreas.map((a) => (
                <div key={a.value} className="flex items-center gap-2">
                  ...
                  <span className="text-sm">
                    {t.ranking?.areas?.[
                      a.key as keyof typeof t.ranking.areas
                    ] ?? a.value}
                  </span>
                </div>
              ))}
            ...
            <div>
              ...
              {productFlags.map((p) => (
                <div key={p} className="flex items-center gap-2">
                  ...
                  <span className="text-sm">
                    {
                      t.admin.shopForm.extras.productTags[
                        p
                          .toLowerCase()
                          .replace(
                            " ",
