...
async function fetchArticle(slug: string): Promise<Article | null> {
  const { data, error } = await supabaseAdmin
    ...
    .select(
      ...
      thumbnail_url, published_at, updated_at, status, is_protected, password_hash, shopify_url,
...
export default async function ArticleDetailPage({ params }: Props) {
    ...
    if (!unlocked) {
      return (
        <PasswordGate
          ...
          shopifyUrl={article.shopify_url as string}
        />
      );
    }
  }
  ...
  // ── Render article ────────────────────────────────────────────────────────
  const tags =
    article.article_tags?.map((at) => at.general_blog_tags).filter(Boolean) ??
    [];
  ...
  return (
    <main className="bg-[#FAF8F4] min-h-screen font-sans text-[#0F0F0F]">
      ...
      />
      {/* ── Hero ── */}
      <div className="relative w-full h-130 sm:h-95 overflow-hidden bg-[#0F0F0F]">
        {article.thumbnail_url ? (
          ...
          <div className="absolute inset-0 bg-linear-to-br from-[#1a1a2e] via-[#16213e] to-[#0f0f0f]" />
        )}

        <div className="absolute inset-0 bg-linear-to-b from-transparent via-black/30 to-black/80" />

        <div className="absolute bottom-0 left-0 right-0 px-[clamp(1.25rem,5vw,4rem)] pb-10 max-w-225 mx-auto">
          <Link
            href={`/${locale}/blog`}
          ...
          {article.blog_categories && (
            <div>
              <Link
                ...
                className={`inline-block text-[0.7rem] font-semibold tracking-[0.09em] uppercase px-3 py-1 rounded-xs mb-4 ${
                  categoryColors[
          ...
          <div className="flex items-center gap-3 mb-4 mt-2">
            ...
            {article.is_protected && (
              <span className="shrink-0 mt-1.5 inline-flex items-center gap-1 bg-white/10 text-white/70 text-[0.65rem] font-medium tracking-wider uppercase px-2 py-0.5 rounded border border-white/20">
          ...
          {article.published_at && (
            <div className="flex items-center gap-2 text-white/50 text-[0.8rem] font-normal tracking-wide flex-wrap">
              <span>{t.blogArticle.hero.author}</span>
              <span>TCG Finder Japan</span>
      ...
      {/* ── Body ── */}
      <div className="max-w-275 mx-auto px-[clamp(1.25rem,5vw,4rem)] pb-20">
        {/* Breadcrumb */}
        <nav
        ...
        >
          ...
          <span className="text-[#E8E3DB] text-sm">›</span>
          <span className="text-[#6B7C6E] max-w-55 whitespace-nowrap overflow-hidden text-ellipsis">
            {article.title}
          </span>
        </nav>
        ...
        {/* Excerpt */}
        <blockquote className="font-serif text-[clamp(1rem,2.2vw,1.2rem)] font-normal text-[#6B7C6E] leading-[1.75] px-7 py-6 mb-10 border-l-[3px] border-[#C8861A] bg-[#C8861A]/4 rounded-r-lg">
          {article.excerpt}
        </blockquote>
        ...
        <div className="text-[clamp(0.9rem,1.8vw,1rem)] leading-[1.85] p-0 text-[#2C2C2C] whitespace-pre-wrap bg-white border border-[#E8E3DB] rounded-xl px-[clamp(1.5rem,4vw,3rem)] py-10 shadow-sm mb-10">
          <div
            className="p-[0!important] border-0 shadow-[none!important] prose prose-stone max-w-none
             ...
             prose-blockquote:border-l-[#C8861A] prose-blockquote:text-[#6B7C6E] prose-blockquote:bg-[#C8861A]/4 prose-blockquote:rounded-r-lg
...
prose-code:bg-[#F4F1EC] prose-code:text-[#C8861A] prose-code:px-1.5 prose-code:py-0.5 prose-code:rounded prose-code:text-[0.85em] prose-code:font-mono prose-code:before:content-none prose-code:after:content-none
             ...
             bg-white border-[#E8E3DB] rounded-xl px-[clamp(1.5rem,4vw,3rem)] py-10 mb-10"
            dangerouslySetInnerHTML={{
            ...
            <div className="flex flex-wrap gap-2">
              {tags.map((tag) => (
                <Link
                  ...
                  className="inline-block px-3.5 py-1.5 text-[0.75rem] font-medium text-[#6B7C6E] border border-[#E8E3DB] rounded-full bg-transparent hover:border-[#C8861A] hover:text-[#C8861A] hover:bg-[#C8861A]/6 transition-all duration-150 tracking-[0.01em]"
                >
                  {tag!.name}
                </Link>
              ))}
